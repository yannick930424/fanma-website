...
import { CookieCategories } from '../i18n/translations/cookies';

type CategoryKey = keyof CookieCategories;
...
export default function CookieConsentBanner() {
  ...
  const saveSettings = (newSettings: CookieSettings) => {
    ...
    if (settingsToSave.analytics && window.gtag) {
      window.gtag('consent', 'update', {
        'analytics_storage': 'granted'
      });
    }

    if (settingsToSave.marketing && window.gtag) {
      window.gtag('consent', 'update', {
        'ad_storage': 'granted'
      });
    }
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        {isExpanded && (
          <div className="space-y-4 mb-6">
            {(Object.entries(t.categories) as [CategoryKey, CookieCategories[CategoryKey]][]).map(([key, category]) => (
